...
### Возможности
- Подключение устройства ACE по Serial (автодетект VID/PID и описания).
...
- Устройство не находится:
  - Проверьте путь `serial` в секции `[ace]`.
  - Оставьте `serial` пустым — задействуется автодетект по VID/PID/описанию.
